...
function buildCreateServer(route: any) {
  return createRouteTestServer((nextRequest) => route.POST(nextRequest));
}
...
describe("client meal plan lifecycle & immutability (supertest + jest)", () => {
  ...
  it("deduplicates a replayed draft create operation", async () => {
    const { client, dietitian } = await createAssignedDietitianClientPair();
    (getServerSession as jest.Mock).mockResolvedValue({
      user: toSessionUser(dietitian),
    });
    const route = await import("@/app/api/client-meal-plans/route");
    const server = buildCreateServer(route);
    const operationId = "draft-create-operation-1234";
    ...
    try {
      const first = await request(server)
        ...
        .set("x-idempotency-key", operationId)
        .send(payload);
      ...
      expect(replay.body.mealPlan._id).toBe(first.body.mealPlan._id);
      await expect(
        ClientMealPlan.countDocuments({ operationId }),
      ).resolves.toBe(1);
    } finally {
      server.close();
    }
  ...
  it("returns the committed publish when the same operation is replayed", async () => {
    const { client, dietitian } = await createAssignedDietitianClientPair();
    const plan = await createDraftPlan({
      clientId: client._id,
      dietitianId: dietitian._id,
    ...
    const planId = entityId(plan);
    const server = buildPutServer(planId, route);
    const operationId = "publish-operation-1234";
    ...
    try {
      ...
      expect(fresh.republishCount).toBe(1);
      expect(
        fresh.lifecycleAudit.filter((entry: any) => entry.action === "publish"),
      ).toHaveLength(1);
      expect(fresh.lastOperationId).toBe(operationId);
    } finally {
      server.close();
    }
  });
  ...
  it("blocks title edit on a published plan with 409 TITLE_LOCKED_AFTER_PUBLISH", async () => {
    ...
    try {
      ...
      expect(
        fresh.lifecycleAudit.some(
          (e: any) => e.action === "blocked_title_edit",
        ),
      ).toBe(true);
    } finally {
      server.close();
  ...
  it("blocks active -> draft transition with 409 FORBIDDEN_STATE_TRANSITION", async () => {
    ...
    try {
      ...
      expect(
        fresh.lifecycleAudit.some(
          (e: any) => e.action === "blocked_revert_to_draft",
        ),
      ).toBe(true);
    } finally {
      server.close();
  ...
  it("republishing same plan id increments republishCount and preserves firstPublishedAt", async () => {
    ...
    try {
      ...
      expect(new Date(afterSecond.firstPublishedAt).toISOString()).toBe(
        new Date(firstPublishedAt).toISOString(),
      );
  ...
  it("returns 403 for an unrelated dietitian attempting update", async () => {
    ...
    (getServerSession as jest.Mock).mockResolvedValue({
      user: toSessionUser(otherDietitian),
    });
    const route = await import("@/app/api/client-meal-plans/[id]/route");
    const planId = entityId(plan);
    const server = buildPutServer(planId, route);
  ...
  it("planning list with status=all returns drafts + active plans for assigned dietitian", async () => {
    ...
    const activePlan = await createDraftPlan({
      clientId: client._id,
      dietitianId: dietitian._id,
      name: "Active One",
    });
    ...
    const server = createRouteTestServer((nextRequest) =>
      listRoute.GET(nextRequest),
    );

    try {
      const res = await request(server).get(
        `/api/client-meal-plans?clientId=${entityId(client)}&status=all&limit=200`,
      );
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
      const names = res.body.mealPlans.map((p: any) => p.name).sort();
      expect(names).toEqual(["Active One", "Draft One"]);
      const active = res.body.mealPlans.find(
        (p: any) => p.name === "Active One",
      );
