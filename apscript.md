var API_URL = "https://dtpoonamsagar.com/api/orders?status=any&per_page=100";
...
function cleanPhoneNumber(phone) {
  ...
  // Remove country codes and extensions from the start:
  // +91 (India), 91, +1 (USA), 1, +44 (UK), 44, etc.
...
function orderToRow(order) {
  return [
    ...
    cleanPhoneNumber(order.customerPhone),  // Cleans +91, 91 and other country codes
    safe(order.city),
...
function fetchOrders() {
  var resp = UrlFetchApp.fetch(API_URL, {
    ...
    headers: {
      ...
      "User-Agent": "Google-Apps-Script Orders Sync/1.0",
  ...
  try {
    ...
    throw new Error("Failed to parse JSON response: " + e.message + " | " + body.substring(0, 700));
  }

  if (!json || json.success !== true || !Array.isArray(json.orders)) {
    throw new Error("Unexpected API response. Expected { success: true, orders: [] }; got: " + JSON.stringify(json).substring(0, 700));
  }

  Logger.log("✅ Fetched " + json.orders.length + " orders from API");
  return json.orders;
}
...
function syncOrdersToExistingSheets() {
  ...
  try {
    ...
    Object.keys(grouped).forEach(function(status) {
      ...
      rows.sort(function(a, b) { return new Date(a[10]) - new Date(b[10]); });
      grouped[status] = rows;
    });

    appendNewRowsOnly(getOrCreateSheet(ss, SHEETS.completed), grouped.completed);
    appendNewRowsOnly(getOrCreateSheet(ss, SHEETS.failed), grouped.failed);
    appendNewRowsOnly(getOrCreateSheet(ss, SHEETS.cancelled), grouped.cancelled);
...
function setupTrigger() {
  ...
  // Create trigger using cron job schedule
  ...
  Logger.log("✅ 5-minute cron job trigger installed for syncOrdersToExistingSheets");
  Logger.log("Schedule: Every 5 minutes (*/5 * * * *)");
  Logger.log("Function: syncOrdersToExistingSheets");
}
...
function checkTriggerStatus() {
  ...
  if (active.length > 0) {
    ...
    Logger.log("   Found " + active.length + " trigger(s)");
    Logger.log("   Schedule: Every 5 minutes (*/5 * * * *)");
    Logger.log("   Function: syncOrdersToExistingSheets");
    Logger.log("   Next run: Check Google Apps Script Executions tab");
  } else {
    Logger.log("❌ No active trigger found");
    Logger.log("   Run setupTrigger() to install one");
  }
}

function manualSync() {
  Logger.log("🔄 Manual sync started @ " + formatIST(new Date()));
  syncOrdersToExistingSheets();
  Logger.log("🔄 Manual sync completed @ " + formatIST(new Date()));
}
...
function cleanAllPhoneNumbers() {
  ...
  sheets.forEach(function(sh) {
    var lastRow = sh.getLastRow();
    if (lastRow <= 1) return; // Skip empty sheets or header only

    // Phone number is in column 5 (Customer Phone)
    ...
    var cleanedValues = phoneValues.map(function(row) {
      ...
      var cleaned = String(phone).trim();
      // Remove +91 or 91 from the start
  ...
  var msg = "Phone number cleanup complete! Cleaned " + totalCleaned + " phone numbers.";
  Logger.log(msg);
