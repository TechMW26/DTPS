...
class SocketClient {
  ...
  private _broadcastToListeners(event: string, data: any): void {
  ...
  private scheduleReconnect(): void {
    ...
    if (this.retryCount >= MAX_RETRIES) {
      console.warn("[SocketClient] Max retries exceeded, socket is DOWN");
      this._down = true;
      ...
      this._broadcastToListeners(SOCKET_EVENTS.SOCKET_DOWN, {
        message:
          "Socket.io connection lost after max retries. Real-time features unavailable.",
        timestamp: Date.now(),
      });
      return;
    }
