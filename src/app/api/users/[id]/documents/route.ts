...
export async function POST(
  ...
  try {
    ...
    // Resolve the target and permissions before creating any remote media.
    ...
    const isDietitianAssigned =
      ...
      (user.assignedDietitian?.toString() === session.user.id ||
        user.assignedDietitians?.some(
          (d: { toString: () => string }) => d.toString() === session.user.id,
        ));
    const isHCAssigned =
    ...
    if (
      ...
      try {
        ...
        console.error(
          "Error sending meal-picture notifications:",
          notificationError,
        );
      }
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (documentIndex === null && !filePath) {
      return NextResponse.json(
        { error: "Document index or filePath required" },
        { status: 400 },
      );
    }
    ...
    if (filePath && docIndex === -1) {
      docIndex =
        user.documents?.findIndex((d: any) => d.filePath === filePath) ?? -1;
    }

    if (
      docIndex === -1 ||
      !user.documents ||
      docIndex >= user.documents.length
    ) {
      return NextResponse.json(
