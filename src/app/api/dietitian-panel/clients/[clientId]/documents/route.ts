...
function extractMealType(raw: string): string {
  ...
  const sortedKeys = Object.keys(MEAL_TYPE_MAP).sort(
    (a, b) => b.length - a.length,
  );
  for (const key of sortedKeys) {
    if (
      withoutExt.startsWith(key) ||
      withoutExt.toLowerCase().startsWith(key.toLowerCase())
    ) {
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        { error: "Forbidden - Dietitian/Admin access required" },
        { status: 403 },
      );
    }
    ...
    const user = await withCache<ClientUserLite | null>(
      ...
      async () =>
        await User.findById(clientId)
          .select(
            "documents firstName lastName assignedDietitian assignedDietitians",
          )
    ...
    const assembled = await withCache(
      ...
      async () => {
        ...
        const [medicalInfo, mealPlans, transformationFiles, completeMealFiles] =
          await Promise.all([
            MedicalInfo.findOne({ userId: clientId })
              .lean<MedicalInfoLite | null>()
              .catch(() => null),
            ClientMealPlan.find({
              clientId,
          ...
          completions.forEach((completion) => {
            if (completion.imagePath) {
              ...
              const mealTypeDisplay =
                completion.mealTypeOriginal ||
                (completion.mealType || "Meal")
                  .split("_")
                  .map(
                    (word: string) =>
          ...
          if (transformationFiles.length > 0) {
            ...
            transformationImages = clientTransformationFiles.map((file) => {
              ...
              return {
                id:
                  file.fileId ||
                  `transformation-${Date.now()}-${Math.random()}`,
    ...
    const mealPicturesCount = uniqueDocuments.filter(
      (d) => d.type === "meal-picture",
    ).length;
    const medicalReportsCount = uniqueDocuments.filter(
      (d) => d.type === "medical-report",
    ).length;
    const transformationsCount = uniqueDocuments.filter(
      (d) => d.type === "transformation",
    ).length;
