...
const slowApiThresholdMs = parseInt(
  process.env.SLOW_API_THRESHOLD_MS || "4000",
  10,
);
const criticalSlowApiThresholdMs = parseInt(
  process.env.CRITICAL_SLOW_API_THRESHOLD_MS || "8000",
  10,
);
...
function resolveSection(pathname, role) {
  ...
  if (
    pathname.startsWith("/api/dietitian-panel") ||
    pathname.startsWith("/api/dietician-panel")
  )
    return "dietitian";
  ...
  if (normalizedRole === "dietitian" || normalizedRole === "dietician")
    return "dietitian";
  if (
    normalizedRole === "health_counselor" ||
    normalizedRole === "health-counselor"
  )
...
async function postRuntimeAlert(payload) {
  try {
    await fetch(
      new URL("/api/internal/runtime-alert", runtimeMonitorBaseUrl).toString(),
      {
        method: "POST",
        headers: {
    ...
    const onFinish = () => {
      ...
      void (async () => {
        ...
        const ipAddress = Array.isArray(ipHeader)
          ? ipHeader[0]
          : String(ipHeader || req.socket?.remoteAddress || "")
              .split(",")[0]
              .trim();
        ...
        await postRuntimeAlert({
          ...
          priority: isServerError
            ? statusCode >= 503
              ? "critical"
              : "high"
            : durationMs >= criticalSlowApiThresholdMs
              ? "high"
              : "medium",
          ...
          details: {
            ...
            clientName:
              section === "client" ? actor?.name || undefined : undefined,
            ipAddress: ipAddress || undefined,
            userAgent: req.headers["user-agent"] || undefined,
            query:
              parsedUrl.query && Object.keys(parsedUrl.query).length > 0
                ? parsedUrl.query
                : undefined,
